...
class WhatsAppAccount(TimeStampedModel):
    ...
    phone_number_id = models.CharField(
        max_length=50,
        unique=True,
        help_text='Phone Number ID de Meta'
    )
...
class WhatsAppTemplate(TimeStampedModel):
    ...
    name = models.CharField(
        max_length=255,
        help_text='Nombre de la plantilla (ej: hello_world)'
    )
    language = models.CharField(
        max_length=10,
        help_text='Código de idioma (ej: es, en_US)'
    )
    ...
    components = models.JSONField(
        default=dict,
        blank=True,
        help_text='Estructura de la plantilla (header, body, footer, buttons)'
    )
