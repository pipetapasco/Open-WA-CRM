...
class Conversation(TimeStampedModel):
    ...
    last_message_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text='Fecha del último mensaje para ordenar inbox'
    )
...
class Message(TimeStampedModel):
    ...
    whatsapp_id = models.CharField(
        max_length=255, unique=True, db_index=True, help_text='wamid de WhatsApp (identificador único del mensaje)'
    )
    direction = models.CharField(max_length=10, choices=Direction.choices)
    message_type = models.CharField(max_length=20, choices=MessageType.choices, default=MessageType.TEXT)
    body = models.TextField(blank=True, null=True, help_text='Contenido del mensaje (para mensajes de texto)')
    media_url = models.URLField(max_length=500, blank=True, null=True, help_text='URL del archivo adjunto')
    metadata = models.JSONField(default=dict, blank=True, help_text='Payload completo del mensaje de Meta')
